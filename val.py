...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            # Evaluate
            if nl:
                tbox = xywh2xyxy(labels[:, 1:5])  # target boxe
    ...
    with open(f'./{save_dir}/results.txt', 'w') as f:
        print(confusion_matrix.matrix, file=f)
        print()
        print(s, file=f)
        pf = '%20s' + '%11i' * 2 + '%11.3g' * 4  # print format
        ...
        print(f'{names[0]} : {f1[0]}', file=f)
        print(f'{names[1]} : {f1[1]}', file=f)
        print(f'{names[2]} : {f1[2]}', file=f)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='data/femoral_artery_512.yaml', help='dataset.yaml path')
